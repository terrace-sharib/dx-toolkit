...
def set_details(args):
    ...
    if args.details is not None and args.details_file is not None:
        parser.exit(1,fill('Error: -f/--details-file and JSON cannot be simultaneously specified.')+'\n')
    ...
    ##Throw error if missing arguments.
        ...
        parser.exit(1,fill('Error: Must set one of -f/--details-file or details.')+'\n')
    
    for result in entity_results:
        try:
            dxpy.DXHTTPRequest('/' + result['id'] + '/setDetails',
                                details)
        except (dxpy.DXAPIError,) + network_exceptions as ex_details:
            print(fill(ex_details.__class__.__name__ + ': ' + str(ex_details)))
            had_error = True
