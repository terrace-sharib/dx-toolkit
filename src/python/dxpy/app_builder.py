...
def create_app(applet_id, applet_name, src_dir, publish=False, set_default=False, billTo=None, try_versions=None, try_update=True):
    ...
    if authorized_users_to_set is not None:
        ...
        authorized_users_to_add = set(authorized_users_to_set).difference(set(existing_authorized_users))
        authorized_users_to_remove = set(existing_authorized_users).difference(set(authorized_users_to_set))
